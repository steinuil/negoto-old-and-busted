...
module NegotoClasses
  ...
  class Board
    def initialize(imgboard, board_id)
      @info = imgboard.info
      @board = imgboard.get_collection(board_id)
      @board_id = board_id
    end
    ...
    def threads
      return @board.find.sort(updated: -1).to_a
    end
  ...
  def format(text)
    ...
    text.map!.with_index do |string, counter|
      if counter.even? # to fix: this only works if the first line is not code
